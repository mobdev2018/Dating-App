...
open class NHVerticalRangeSliderView: UIView {
    ...
    open func setup() {
        ...
        self.lowerLabel?.font = UIFont.systemFont(ofSize: 22.0)
        self.lowerLabel?.text = ""
        self.lowerLabel?.textAlignment = .center
        lowerLabel?.textColor = .sliderLabelColor
        self.addSubview(self.lowerLabel!)
        
        self.leftLowerLabel = UILabel(frame: .zero)
        self.leftLowerLabel?.numberOfLines = 1
        self.leftLowerLabel?.font = UIFont.systemFont(ofSize: 22.0)
        self.leftLowerLabel?.text = ""
        self.leftLowerLabel?.textAlignment = .center
        leftLowerLabel?.textColor = .sliderLabelColor
        self.addSubview(self.leftLowerLabel!)
        
        self.upperLabel = UILabel(frame: .zero)
        self.upperLabel?.numberOfLines = 1
        self.upperLabel?.font = UIFont.systemFont(ofSize: 22.0)
        self.upperLabel?.text = ""
        self.upperLabel?.textAlignment = .center
        upperLabel?.textColor = .sliderLabelColor
        self.addSubview(self.upperLabel!)
        
        self.leftUpperLabel = UILabel(frame: .zero)
        self.leftUpperLabel?.numberOfLines = 1
        self.leftUpperLabel?.font = UIFont.systemFont(ofSize: 22.0)
        self.leftUpperLabel?.text = ""
        self.leftUpperLabel?.textAlignment = .center
        leftUpperLabel?.textColor = .sliderLabelColor
        self.addSubview(self.leftUpperLabel!)
        
        self.rangeSlider = NHRangeSlider(frame: .zero)
        self.addSubview(self.rangeSlider!)
        
        bringSubview(toFront: lowerLabel!)
        bringSubview(toFront: upperLabel!)
        
        bringSubview(toFront: leftLowerLabel!)
        bringSubview(toFront: leftUpperLabel!)
        self.updateLabelDisplay()
    ...
    //@IBAction func onValueChange() {
    //}
    
    // update labels display
    open func updateLabelDisplay() {
        self.lowerLabel?.text = cmToFeet(cm: Float(rangeSlider!.lowerValue))
        self.leftLowerLabel?.text = "\(Int(rangeSlider!.lowerValue)) cm"
        self.upperLabel?.text = cmToFeet(cm: Float(rangeSlider!.upperValue))
        self.leftUpperLabel?.text = "\(Int(rangeSlider!.upperValue)) cm"
        
        if self.lowerLabel != nil {
            
            // for stepped value we animate the labels
            if self.stepValue != 1.0 && self.thumbLabelStyle == .FOLLOW {
    ...
    override open func layoutSubviews() {
        ...
        if let titleLabel = self.titleLabel , let lowerLabel = self.lowerLabel ,
            ...
            rangeSlider.center = CGPoint(x: self.bounds.size.width/2, y: self.bounds.size.height/2)
            ...
            lowerLabel.frame = CGRect(      x: lowerLabelX,
                                            y: 20.0 + titleLabelMaxY + rangeSlider.bounds.size.height + self.spacing,
                                            width: lowerWidth ,
                                            height: 100)
            
            leftLowerLabel?.frame = CGRect(  x: lowerLabelX,
                                             y: -100.0,
                                             width: lowerWidth,
                                             height: 100)
            ...
            leftUpperLabel?.frame = CGRect(  x: upperLabelX,
                                             y: -100.0,
                                             width:upperWidth ,
                                             height: 100)
